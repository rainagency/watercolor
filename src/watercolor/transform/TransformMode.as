package watercolor.transform
{

	/**
	 * Available transformation modes.
	 *
	 */
	public class TransformMode
	{
		static public const MODE_IDLE:String = "modeIdle";
		static public const MODE_SCALEX:String = "modeScaleX";
		static public const MODE_SCALEY:String = "modeScaleY";
		static public const MODE_SCALE:String = "modeScale";
		static public const MODE_SKEWX:String = "modeSkewX";
		static public const MODE_SKEWY:String = "modeSkewY";
		static public const MODE_SKEW:String = "modeSkew";
		static public const MODE_ROTATE:String = "modeRotate";
		static public const MODE_CENTER_POINT:String = "modeRevPoint";
		static public const MODE_MOVE:String = "modeMove";
		static public const MODE_FLIP:String = "modeFlip";
		static public const MODE_NUDGE:String = "modeNudge";
		static public const MODE_MOVETO:String = "modeMoveTo";
	}
}